...
// 1. Added Variant Interface
interface Variant {
  name: string;
  price: number;
}

// 2. Updated MenuItem to include optional variants
interface MenuItem {
  id: string;
  name: string;
  price: number;
...
export default function RestaurantPage({ params }: PageProps) {
  ...
  // 3. States for managing the variant selection modal
  ...
  // Handle adding items to cart or opening the variant modal
  const handleItemClick = (item: MenuItem, catName: string) => {
    if (!isShopOpen) return; // Block all interaction when closed

    if (item.variants && item.variants.length > 0) {
      // Open modal and set default variant to the first one
  ...
  // Add the specifically chosen variant to the cart
  const confirmVariantAdd = () => {
    if (!isShopOpen) return;
    if (selectedItem && selectedVariant) {
      addItem({
        ...selectedItem,
        id: `${selectedItem.id}-${selectedVariant.name}`, // Create unique ID for the cart
        name: `${selectedItem.name} (${selectedVariant.name})`, // Append size to name
          ...
          <section key={cat.name} id={cat.name} className="mb-12">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {cat.items.map((item: MenuItem) => {
                // Calculate minimum price if variants exist
                ...
                return (
                  <div
                    ...
                    onKeyDown={(e) => {
                      if ((e.key === "Enter" || e.key === " ") && isShopOpen) handleItemClick(item, cat.name);
                    }}
                    className={`bg-white rounded-2xl shadow-sm border border-gray-100 overflow-visible transition-all duration-200 flex flex-col relative ${
                      isShopOpen
                        ? "hover:shadow-lg cursor-pointer active:scale-[0.98]"
                        : "cursor-not-allowed"
                    }`}
                  >
                    {/* Image with overlapping plus button, Foodpanda style */}
                    <div className={`relative w-full aspect-square rounded-t-2xl overflow-hidden bg-gray-100 ${!isShopOpen ? "grayscale opacity-60" : ""}`}>
                      {item.image ? (
                        <Image
                          src={item.image}
                          alt={item.name}
                      ...
                      <button
                        ...
                        className={`absolute -bottom-4 left-1/2 -translate-x-1/2 w-9 h-9 rounded-full text-white font-black text-lg flex items-center justify-center shadow-lg border-4 border-white transition-all ${
                          isShopOpen
          ...
          <div className="bg-white w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in slide-in-from-bottom-10 sm:slide-in-from-bottom-0 shadow-2xl">
            ...
            <div className="p-6 overflow-y-auto">
              <div className="flex justify-between items-start mb-2">
                ...
                <button onClick={() => setSelectedItem(null)} className="p-2 bg-gray-100 hover:bg-gray-200 transition-colors rounded-full text-gray-500">
